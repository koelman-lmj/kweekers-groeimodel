...
function getHeaderChecks(workbook: ExcelJS.Workbook) {
  return Object.entries(requiredSheets).map(([sheetName, requiredColumns]) => {
    const sheet = workbook.getWorksheet(sheetName);
...
function buildConcept(recordsBySheet: Record<string, RowRecord[]>) {
  return {
    status: "concept",
    createdAt: new Date().toISOString(),
    data: {
      categories: (recordsBySheet.categories ?? []).map((row) => row.record),
      dimensions: (recordsBySheet.dimensions ?? []).map((row) => row.record),
      questions: (recordsBySheet.questions ?? []).map((row) => row.record),
      optionSets: (recordsBySheet.optionSets ?? []).map((row) => row.record),
      options: (recordsBySheet.options ?? []).map((row) => row.record),
    },
  };
}
...
export async function POST(request: Request) {
  try {
    ...
    const checks = getHeaderChecks(workbook);
      ...
      return NextResponse.json(
        {
          ok: false,
          message:
            "Concept kan niet worden aangemaakt omdat het Excelbestand nog fouten bevat.",
          checks,
          duplicateIssues,
          requiredFieldIssues,
          relationIssues,
    ...
    const concept = buildConcept(recordsBySheet);

    return NextResponse.json({
      ok: true,
      message:
        "Concept-import is voorbereid. Er is nog niets definitief gepubliceerd.",
      stored: false,
      fileName: file.name,
      importSummary,
      concept,
